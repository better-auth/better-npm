...
type Step = "loading" | "confirm" | "signing-in" | "approving" | "done" | "error";
...
export default function DeviceAuthPage() {
  ...
  useEffect(() => {
    ...
    setUserCode(params.get("user_code") || params.get("code"));
  }, []);

  useEffect(() => {
    if (isPending || !userCode) return;

    if (session && !approvedRef.current) {
  ...
  async function approveDevice() {
    ...
    try {
      const { error: approveError } = await authClient.device.approve({
        userCode: userCode.replace(/-/g, "").toUpperCase(),
      });
  ...
  function handleGitHubLogin() {
    ...
    signIn.social({
      provider: "github",
      callbackURL: `/auth/device?user_code=${userCode}`,
    });
  }
    ...
    return (
      <Shell>
        ...
        <p className="mt-3 text-sm text-foreground/50">
          Run <code className="font-mono text-foreground/60">npx @better-npm/cli</code> in
    ...
    return (
      <Shell>
        ...
        <p className="mt-4 text-sm text-foreground/40">
          Run <code className="font-mono text-foreground/60">npx @better-npm/cli</code> to
          try again.
        </p>
      </Shell>
    );
  ...
  return (
    <Shell>
      ...
      <div className="mt-8 px-6 py-4 border border-foreground/[0.08] rounded bg-foreground/[0.02] text-center">
